...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Regenerate images conditioned on previous outputs using Bagel model.")
    parser.add_argument("--output_dir", type=str, required=True, help="Directory to save regenerated images.")
    # Input source: choose either JSONL (--metadata_file) or CSV (--prompt_dir).
    parser.add_argument("--metadata_file", type=str, required=False, help="JSONL with per-prompt metadata.")
    parser.add_argument("--prompt_dir", type=str, required=False, help="CSV file path. Preferred columns: 'prompt_en'/'prompt_zh' and 'index'.")
    parser.add_argument("--input_base_dir", type=str, required=True, help="Input image base directory. CSV mode expects flat naming like {index}_{j}.png.")
    ...
    parser.add_argument('--model-path', dest='model_path', type=str, default='./models/RvR-7B-MoT')
    parser.add_argument('--model_path', dest='model_path', type=str, help=argparse.SUPPRESS)
    ...
    if args.prompt_dir and args.metadata_file:
        raise ValueError("Provide only one of --prompt_dir (CSV) or --metadata_file (JSONL).")
    if not args.prompt_dir and not args.metadata_file:
        raise ValueError("Provide one of --prompt_dir (CSV) or --metadata_file (JSONL).")

    metadatas = []
    csv_mode = bool(args.prompt_dir)
    if csv_mode:
        csv_path = args.prompt_dir
        with open(csv_path, "r", encoding="utf-8") as fp:
            ...
            if has_header:
                ...
                if "prompt_en" in fieldnames:
                    prompt_col = "prompt_en"
                elif "prompt_zh" in fieldnames:
                    prompt_col = "prompt_zh"
                elif "prompt" in fieldnames:
                    prompt_col = "prompt"
                else:
                    # Fallback: prefer columns containing 'prompt'/'text', else first non-index column.
                    lowered = {name: str(name).strip().lower() for name in fieldnames}
                    prompt_like_cols = [
                        name for name in fieldnames
                        if ("prompt" in lowered[name]) or ("text" in lowered[name])
                    ]
                    non_index_cols = [
                        name for name in fieldnames
                        if lowered[name] not in {"index", "idx", "id"}
                    ]
                    if prompt_like_cols:
                        prompt_col = prompt_like_cols[0]
                    elif non_index_cols:
                        prompt_col = non_index_cols[0]
                    else:
                        prompt_col = fieldnames[0] if len(fieldnames) > 0 else None
                index_col = "index" if "index" in fieldnames else None
                if prompt_col is None:
                    raise ValueError(f"CSV is missing a usable prompt column: {csv_path}")
                for ridx, row in enumerate(reader):
                    prompt_val = (row.get(prompt_col) or "").strip()
                    if not prompt_val:
                        continue
                    if index_col is not None:
                        try:
                            index_val = int(row.get(index_col))
                        except Exception:
                            index_val = ridx
                    else:
                        index_val = ridx
                    metadatas.append({"prompt": prompt_val, "index": index_val})
            else:
                fp.seek(0)
                ...
                for ridx, row in enumerate(reader):
                    if not row:
                        continue
                    prompt_val = (row[0] or "").strip()
                    if prompt_val:
                        metadatas.append({"prompt": prompt_val, "index": ridx})
    else:
    ...
    print(f"GPU {rank}: Processing {end - start} items (indices {start} to {end - 1})")

    # CSV mode: flat naming {index}_{j}.png for both condition and output images.
    if csv_mode:
        print(f"CSV mode: reading condition images from {args.input_base_dir} into {output_dir}")
        for local_idx in range(start, end):
            metadata = metadatas[local_idx]
            prompt = metadata["prompt"]
            index_val = int(metadata.get("index", local_idx))
            print(f"GPU {rank} processing index {index_val}: '{prompt}'")

            all_exist = True
            for j in range(args.num_images):
                out_path = os.path.join(output_dir, f"{index_val}_{j}.png")
                if not os.path.exists(out_path):
                    all_exist = False
                    break
            if all_exist:
                print(f"GPU {rank} skipping regeneration for index {index_val} (all outputs exist)")
                continue

            for j in range(args.num_images):
                cond_path = os.path.join(args.input_base_dir, f"{index_val}_{j}.png")
                if not os.path.exists(cond_path):
                    print(f"GPU {rank} missing condition image: {cond_path}, skip this sample")
                    continue
                try:
                    cond_image = Image.open(cond_path).convert("RGB")
                except Exception as e:
                    print(f"GPU {rank} failed to open condition image {cond_path}: {e}")
                    continue
                ...
                # No crop; save the image directly to match the gen-script behaviour.
                out_path = os.path.join(output_dir, f"{index_val}_{j}.png")
                try:
                    gen_image.save(out_path)
                except Exception as e:
                    print(f"GPU {rank} failed to save {out_path}: {e}")
    else:
        # JSONL mode: per-prompt directory layout, samples saved under <out>/{idx:05}/samples.
        # Condition images: <input_base>/images/{idx:05}/samples/*.png
        base_images_dir = os.path.join(args.input_base_dir, "images")
        ...
        for idx in range(start, end):
            ...
            prompt = metadata['prompt']
            print(f"GPU {rank} processing item {idx - start + 1}/{end - start}: '{prompt}'")
            ...
            for k in range(args.num_images):
                if not os.path.exists(os.path.join(sample_path, f"{k:05}.png")):
                    flag = False
                    break
            if flag:
            ...
            cond_dir = os.path.join(base_images_dir, f"{idx:05}", "samples")
            if not os.path.isdir(cond_dir):
                print(f"GPU {rank} missing condition dir: {cond_dir}, skip")
                continue
            ...
            for name in sorted(os.listdir(cond_dir)):
                if name.endswith('.png'):
                    try:
                        img = Image.open(os.path.join(cond_dir, name)).convert('RGB')
                        cond_images.append(img)
                    except Exception:
                        continue

            if len(cond_images) == 0:
                print(f"GPU {rank} no condition images under: {cond_dir}, skip")
                continue

            image_list = []

            # Cycle through condition images when there are fewer than needed.
            needed = args.num_images
            use_list = []
            while len(use_list) < needed:
                for im in cond_images:
                    use_list.append(im)
                    if len(use_list) >= needed:
                        break

            for i in range(args.num_images // args.batch_size):
                batch_images = use_list[i * args.batch_size:(i + 1) * args.batch_size]
                for cond_image in batch_images:
                    gen_image = generate_image_from_condition(
                        prompt=prompt,
                        cond_image=cond_image,
                        cfg_text_scale=cfg_text_scale,
    ...
    try:
        ...
        if dist.is_available() and dist.is_initialized():
            # Pass device_ids explicitly to silence NCCL device-unknown warnings.
            try:
