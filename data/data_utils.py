...
def prepare_attention_mask_per_sample(split_lens, attn_modes, device="cpu", split_types=None, vit_attend_future_text=False):
    """
    ...
    # 定向放宽：如果开启 vit_attend_future_text，则允许 ViT split 看到其右侧的文本 split
    if vit_attend_future_text and split_types is not None:
        starts = []
        _csum = 0
        for s in split_lens:
            starts.append(_csum)
        ...
        for i, stype in enumerate(split_types):
            if stype == 'vit':
                r0 = starts[i]
                r1 = r0 + split_lens[i]
                # 允许该 ViT split attend 到其右侧所有 text split
                for j in range(i + 1, len(split_lens)):
                    if split_types[j] == 'text':
                        c0 = starts[j]
                        c1 = c0 + split_lens[j]
                        attention_mask[r0:r1, c0:c1] = True
