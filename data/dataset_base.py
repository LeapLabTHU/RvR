...
class DataConfig:
    def __init__(
        ...
        self.vit_attend_future_text = vit_attend_future_text
...
class PackedDataset(torch.utils.data.IterableDataset):
    ...
    def build_datasets(self, datasets_metainfo, data_status):
        ...
        is_mandatory = []
        grouped_weights = []
        # grouped_dataset_name is the top-level group key (e.g., rvr, vlm_sft, blip3o);
        # each group may contain multiple sub-datasets keyed by dataset_name.
        for grouped_dataset_name, dataset_args in datasets_metainfo.items():
            ...
            for item in dataset_names:
                ...
                if "parquet_info_path" in meta_info.keys():
                    ...
                    # parquet_info.json may store relative parquet paths
                    # (release-portable). Anchor them to the active release
                    # root so dataset code can look up by absolute path.
                    from .data_paths import resolve_data_path
                    parquet_info = {
                        resolve_data_path(k): v for k, v in parquet_info.items()
                    }
    ...
    def pack_sequence(self, sample, sequence_status):
        ...
        if not self.use_flex:
            sequence_status['nested_attention_masks'].append(
                prepare_attention_mask_per_sample(
                    split_lens, attn_modes, split_types=split_types, vit_attend_future_text=self.data_config.vit_attend_future_text
                )
            )
        else:
