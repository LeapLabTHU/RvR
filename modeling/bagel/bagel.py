...
class Bagel(PreTrainedModel):
    ...
    def prepare_vit_and_text_prefill(self, curr_kvlens, curr_rope, image, text, vit_transforms, tokenizer, new_token_ids):
        packed_vit_tokens, packed_vit_position_ids = list(), list()
        ...
        for curr_kvlen, curr_position_id in zip(curr_kvlens, curr_rope):
            ...
            # ViT patches
            image_tensor = vit_transforms(image)
            ...
            # 用户文本（含 <|im_start|>/ <|im_end|>）
            text_ids = tokenizer.encode(text)
            text_ids = [new_token_ids['bos_token_id']] + text_ids + [new_token_ids['eos_token_id']]
            packed_text_ids.extend(text_ids)
            packed_text_indexes.extend(range(_curr, _curr + len(text_ids)))
            packed_indexes.extend(range(curr, curr + len(text_ids)))
            curr += len(text_ids)
            _curr += len(text_ids)

            # 位置：ViT 段用同一 position_id，文本段递增
            vit_len_with_2 = num_img_tokens + 2
            text_len_with_2 = len(text_ids)
            packed_position_ids.extend([curr_position_id] * vit_len_with_2)
            packed_position_ids.extend(range(curr_position_id + 1, curr_position_id + 1 + text_len_with_2))

            packed_seqlens.append(vit_len_with_2 + text_len_with_2)
            newlens.append(curr_kvlen + vit_len_with_2 + text_len_with_2)
            new_rope.append(curr_position_id + 1 + text_len_with_2)

        generation_input = {
            "packed_text_ids": torch.tensor(packed_text_ids, dtype=torch.long),
            "packed_text_indexes": torch.tensor(packed_text_indexes, dtype=torch.long),
            "vit_token_seqlens": torch.tensor([num_img_tokens], dtype=torch.int),
    ...
    @torch.no_grad
    def forward_cache_update_vit_text(
        self,
        ...
        # 构造 packed_query_sequence: text embedding + vit embedding 混排
        packed_text_embedding = self.language_model.model.embed_tokens(packed_text_ids)
