...
class PackedAttention(Qwen2Attention):
    ...
    def forward_inference(
        ...
        if custom_attn_mask is None:
            ...
            # 使用 SDPA + 自定义掩码（仅支持单样本场景）
            # 组装 Q/K/V 形状: [1, num_heads, L, head_dim]
            q = packed_query_states.transpose(0, 1)  # [num_heads, Lq, Dh]
            # 重复 kv heads 至 num_heads
            kv = merged_key_states[:, :, None, :].repeat(1, 1, self.num_key_value_groups, 1).reshape(-1, self.num_heads, self.head_dim)
            vv = merged_value_states[:, :, None, :].repeat(1, 1, self.num_key_value_groups, 1).reshape(-1, self.num_heads, self.head_dim)
            q = q.unsqueeze(0)
            k = kv.transpose(0, 1).unsqueeze(0)  # [1, num_heads, Lk, Dh]
            v = vv.transpose(0, 1).unsqueeze(0)  # [1, num_heads, Lk, Dh]
            with sdpa_kernel(backends=[SDPBackend.EFFICIENT_ATTENTION]):
                attn_out = scaled_dot_product_attention(q, k, v, attn_mask=custom_attn_mask.to(q.dtype))
            attn_out = attn_out.squeeze(0).transpose(0, 1).reshape(-1, self.hidden_size)
            packed_attn_output = attn_out
        packed_attn_output = self.o_proj(packed_attn_output)
...
class PackedAttentionMoT(Qwen2Attention):
    ...
    def forward_inference(
        ...
        if custom_attn_mask is None or mode != 'und':
            packed_attn_output = flash_attn_varlen_func(
                q=packed_query_states,
                k=merged_key_states,
                v=merged_value_states,
            ...
            # 使用 SDPA + 自定义掩码（仅 mode=='und'）
            q = packed_query_states.transpose(0, 1)  # [num_heads, Lq, Dh]
            kv = merged_key_states[:, :, None, :].repeat(1, 1, self.num_key_value_groups, 1).reshape(-1, self.num_heads, self.head_dim)
            vv = merged_value_states[:, :, None, :].repeat(1, 1, self.num_key_value_groups, 1).reshape(-1, self.num_heads, self.head_dim)
            q = q.unsqueeze(0)
            k = kv.transpose(0, 1).unsqueeze(0)
            v = vv.transpose(0, 1).unsqueeze(0)
            with sdpa_kernel(backends=[SDPBackend.EFFICIENT_ATTENTION]):
                attn_out = scaled_dot_product_attention(q, k, v, attn_mask=custom_attn_mask.to(q.dtype))
            attn_out = attn_out.squeeze(0).transpose(0, 1).reshape(-1, self.hidden_size)
            packed_attn_output = attn_out
