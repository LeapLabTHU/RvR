...
class TrainingArguments:
    ...
    vit_attend_future_text: bool = field(
        default=False,
        metadata={"help": "If True, relax mask so ViT splits can attend to right-side text splits."}
    )
...
def main():
    ...
    # Setup packed dataloader. dataset_config_file is e.g. data/configs/rvr.yaml.
    ...
    # 可选：为 ViT 放宽右侧文本可见性
    dataset_config.vit_attend_future_text = training_args.vit_attend_future_text
    ...
    for curr_step, data in enumerate(train_loader, start=train_step):
        ...
        if curr_step > 0 and curr_step % training_args.save_every == 0:
            # 确保所有 rank 一致地进入保存阶段，避免 collective 顺序不一致
            dist.barrier()
            if dist.get_rank() == 0:
            ...
            # 等待所有进程完成 gather，再进入 FSDP state_dict 的 all_gather 阶段
            dist.barrier()
