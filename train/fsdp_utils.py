...
class FSDPCheckpoint:
    ...
    def fsdp_save_ckpt(
        ...
        # Entry barrier: ensure all ranks are aligned before triggering FSDP all_gather / state_dict.
        dist.barrier()
        ...
        if ema_model is not None:
            # Synchronize and clear the cache before dumping EMA to reduce GPU memory fragmentation and peak usage.
        ...
        # Synchronize and clear the cache before dumping the model to reduce GPU memory fragmentation and peak usage.
        torch.cuda.synchronize()
        torch.cuda.empty_cache()
        with FSDP.state_dict_type(
            model,
